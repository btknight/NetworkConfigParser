...
def parse_autodetect(doc_lines: List[str]) -> List[DocumentLine]:
    ...
    This method, at present, searches for brace characters '{' and '}' to detect whether parse_braced() should be used.
    If a minimum number of lines is found to have both opening and closing braces at the end, parse_braced() is used.
    Otherwise, the document is assumed to be structured with leading spaces and parse_leading_spaces() is used.
...
def parse_braced(doc_lines: List[str]) -> List[DocumentLine]:
    ...
    This parser should be used for JunOS-like configurations, including but not limited to JunOS itself, UBNT, Versa
    Networks, and Ribbon Neptune. Can be called directly, or can be called via parse_autodetect().
...
def parse_leading_spaces(doc_lines: List[str]) -> List[DocumentLine]:
    ...
    This parser should be used for Cisco-type configurations, including but not limited to Cisco IOS, IOS XE, and IOS
    XR, Arista, and others. Can be called directly, or can be called via parse_autodetect().
    ...
    def current_space_level():
        """Return the current space level as determined by the most recent StackMember at the end of the stack."""
        if len(dn_stack) == 0:
            return 0

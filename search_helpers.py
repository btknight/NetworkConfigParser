...
def convert_search_spec_to_cb(search_spec: SearchTerm | Iterable[SearchTerm],
                              ...
                              term_num: int | None = None) -> Callable[[DocumentLine], bool] | Iterable[Callable[[DocumentLine], bool]]:
    """Converts a find_lines() search term to a callback function.

    Args:
        search_spec:
            User-supplied input to find_lines(). Can be a str, re.Pattern, callable, or iterable of those.
        regex_flags:
    ...
    if is_iterable_search_term(search_spec):
        ...
        return [convert_search_spec_to_cb(t, regex_flags, term_num=i) for i, t in zip(range(len(search_spec)), search_spec)]
    #
    # Handle unsupported terms
    elif not callable(search_spec) and not is_regex(search_spec):
        term_index = f'term index {term_num}: ' if term_num is not None else ''
        raise ValueError(f'convert_search_spec_to_cb: {term_index}{type(search_spec)} is not supported')
    #
...
def find_lines(doc_lines: List[DocumentLine] | None,
    ...
    Search terms may be the following:
        ...
        * A callable function taking a DocumentLine as input and returning a bool to indicate a successful match
        * An iterable containing any combination of the three inputs above

    If the search term is an iterable, each term will be matched in successive generations: if the first term
    matches a top-level object, the second term will be searched in the first term's children or all descendants
    ...
    Args:
        ...
        regex_flags:
            If any search terms are strings or re.Patterns, these flags will be passed to re.search. Default is no
            flags.
        regex_group:
            If the final search term has a group defined in the regex string, this option will return the regex group
            from the re.Match object instead of the DocumentLine object. Default is not to do this. Cannot be combined
            with convert_match.
        recurse_search:
    ...
    if is_iterable_search_term(search_spec):
        ...
        for search_fn in search_spec[:-1]:
            ...
            # Omit the matched line itself ("include_self=False"), get only the children of the match.
            doc_lines = find_lines_with_cb(doc_lines, search_fn, suppress_common_ancestors=False, include_ancestors=False,
...
def find_lines_with_cb(doc_lines: List[DocumentLine],
                       ...
                       include_children: bool = False,
                       include_all_descendants: bool = False) -> List[DocumentLine] | List[List[DocumentLine]]:
    ...
    Args:
        ...
        include_ancestors:
            Set this to True if the ancestors of the matching object should be returned. Default is False.
        include_self:
            Set this to False if the matched line itself should not be returned. Default is True. Used internally when
            find_lines() search_spec is an iterable.
        include_children:
            Set this to True if the immediate children of the matching object should be returned. Default is False.
        include_all_descendants:
            Set this to True if the grandchildren, great-grandchildren, etc. of the matching object should be
            returned. Default is False. Setting this to True implies include_children=True.
...
def parent_child_cb(parent_spec: str | re.Pattern | Callable[[DocumentLine], bool],
    ...
    Returns:
        Search function to feed to find_lines(). Causes find_lines() to return a list of DocumentLines matching the
        parentspec. Children of the objects are not returned.
...
def is_regex(obj: Any) -> bool:
    ...
    """
    return isinstance(obj, str) or isinstance(obj, re.Pattern)
